...
> **Status:** Backend feature-complete. **Phase 0 (Foundation)** and all **5 backend rounds** (Auth, Batches, Dashboard, File details, Errors/Charts, Activity log + validation) are complete, plus **site-level access enforcement (FR-5.3)**. Frontend (DT-3) and stretch goals are next. See [Roadmap & Status](#roadmap--status).
...
Four layers of protection ensure **Customer A can never see Customer B**, and that users only see the **sites they're granted**:
...
4. **Site-access enforcement** — `TenantSiteMiddleware` checks the requested `site_id` against the user's `UserSiteAccess` grants and returns **403 `SITE_FORBIDDEN`** if the user isn't authorized for that site (FR-5.3). This closes the within-tenant gap that the tenant filter alone doesn't cover (a user can no longer read another site in their own tenant just by sending its `X-Site-Id`).
...
> Site access is intentionally **uneven** so you can demonstrate **403 `SITE_FORBIDDEN`**: e.g. `user.b@acme.com` (Chennai only) requesting Mumbai is rejected.
...
8. Log in as the **Globex** user `user.c@globex.com` and confirm you **cannot** see Acme's data — cross-tenant isolation is the headline result.

9. **Site-level isolation:** log in as `user.b@acme.com` (Chennai only) and request a site they lack — set `X-Site-Id` to **Mumbai** — and confirm you get a **403 `SITE_FORBIDDEN`** (FR-5.3).
...
- **Site selection:** global tenant scoping is always enforced; **site is optional** — supplied via `?site_id=` or the `X-Site-Id` header. When no site is supplied, services scope by tenant only. When a site **is** supplied, the middleware validates it against the user's `UserSiteAccess` grants and returns **403 `SITE_FORBIDDEN`** if the user isn't authorized for it.
...
- [x] **Round 1 — Batches list:** `GET /batches` (pagination, filtering, sorting, search)

- [x] **Round 2 —** Dashboard summary / recent failures; Batch detail + files

- [x] **Round 3 —** File details (joins + remediation); Invoice line items

- [x] **Round 4 —** Errors list + CSV export; Charts (throughput, distribution, trend)

- [x] **Round 5 —** Activity log; Input validation + global exception handling

- [x] **Security — Site-level access enforcement (FR-5.3):** `TenantSiteMiddleware` validates `X-Site-Id` against `UserSiteAccess` → **403 `SITE_FORBIDDEN`**.
