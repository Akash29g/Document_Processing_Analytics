...
export class AlertsComponent {
  ...
  // split → tag admins → sort admins first
  ...
  constructor() {
    // load rules + recipients whenever site changes
    effect(() => {
      ...
      untracked(() => {
        this.svc.loadRules();
        this.svc.loadRecipients();
        this.selected.set([]);
        this.menuOpen.set(false);
        this.adminDefaultAppliedForSite = null;
      });
    });

    // once recipients arrive, auto-check logged-in admin only once per site
    effect(() => {
      const siteId = this.site.selectedSiteId();
      const current = this.auth.currentUser();
      const recipients = this.svc.recipients();

      if (!siteId || !current || recipients.length === 0) return;
      if ((current.role ?? '').toLowerCase() !== 'admin') return;
      if (this.adminDefaultAppliedForSite === siteId) return;
      ...
      untracked(() => {
        this.selected.update((list) =>
          list.includes(adminRecipient.email) ? list : [...list, adminRecipient.email],
        );
        this.adminDefaultAppliedForSite = siteId;
      });
    });
  }
  ...
  protected submit(): void {
    if (!this.form.name || this.selected().length === 0) return;

    const payload: AlertRulePayload = {
      name: this.form.name,
      email: this.selected().join(','),
    ...
    // after creating one rule, default-select admin again for the next new rule
    this.adminDefaultAppliedForSite = null;
    this.defaultSelectCurrentAdmin();
  }

  private defaultSelectCurrentAdmin(): void {
    const current = this.auth.currentUser();
