...
export class ProvisioningService {
  ...
  async createTenant(name: string, orgDomain: string): Promise<string | null> {
    try {
      await firstValueFrom(
        this.http.post<ApiResponse<TenantSummary>>(`${this.base}/tenants`, {
          name,
      ...
      return this.msg(e, 'Failed to create tenant.');
    }
  }
  ...
  async createAdmin(tenantId: string, firstName: string, lastName: string): Promise<string | null> {
    try {
      await firstValueFrom(
        this.http.post<ApiResponse<ProvisionedUser>>(`${this.base}/tenants/${tenantId}/admins`, {
      ...
      return this.msg(e, 'Failed to create admin.');
    }
  }
  ...
  async createSite(tenantId: string, name: string, location: string): Promise<string | null> {
    try {
      await firstValueFrom(
        this.http.post<ApiResponse<ProvisionedSite>>(`${this.base}/tenants/${tenantId}/sites`, {
          name,
  ...
  async removeSite(tenantId: string, siteId: string): Promise<string | null> {
    try {
      await firstValueFrom(
        this.http.delete<ApiResponse<unknown>>(`${this.base}/tenants/${tenantId}/sites/${siteId}`),
      );
      await Promise.all([this.loadSites(tenantId), this.loadTenants()]);
      return null;
    } catch (e: any) {
      return this.msg(e, 'Failed to remove site.');
    }
  }
